...
let rec expr_s = 
 ...
 | Assign(v, e) -> v ^ " = " ^ expr_s e (* Maybe add semi colon. Maybe not.*)
 | Noexpr -> ""
...
let rec stmt_s = function
 ...
 | Return(e, vars, is_darr) -> 
		...
		"freeStack(stack);\n"^
		"return" ^ " " ^ expr_s e ^ ";" 
 | If(e, s1, s2) -> "if(" ^ expr_s e ^ ")" ^ stmt_s s1 ^ stmt_s s2
 | For(e1, e2, e3, s) -> "for(" ^ expr_s e1 ^ ";" ^ expr_s e2 ^ ";" ^ expr_s e3 ^ ")" ^ stmt_s s ^ ""
